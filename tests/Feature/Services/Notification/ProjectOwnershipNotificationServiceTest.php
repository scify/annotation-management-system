...
/**
 * Builds a PROJECT_OWNERSHIP thread, optionally with a response row in the given state.
 * With no project owned by the (absent) sender, the member-context lookup resolves to
 * null, so approve/reject simply transition the response and skip ProjectManagerService.
 */
function makeOwnershipThread(?NotificationThreadResponseEnum $response): NotificationThread {
...
describe('ProjectOwnershipNotificationService', function (): void {
    beforeEach(function (): void {
        $this->seed(RolesAndPermissionsSeeder::class);
        $this->service = resolve(ProjectOwnershipNotificationService::class);
    });

    // --- approve() ---

    it('throws when there is no response record to approve', function (): void {
        $thread = makeOwnershipThread(null);
    ...
    it('throws when approving a cancelled thread', function (): void {
        $thread = makeOwnershipThread(NotificationThreadResponseEnum::CANCELED);
    ...
    it('throws when approving an already-rejected thread', function (): void {
        $thread = makeOwnershipThread(NotificationThreadResponseEnum::REJECTED);
    ...
    it('transitions an unreplied thread to accepted', function (): void {
        $thread = makeOwnershipThread(NotificationThreadResponseEnum::UNREPLIED);
    ...
    it('throws when rejecting an already-accepted thread', function (): void {
        $thread = makeOwnershipThread(NotificationThreadResponseEnum::ACCEPTED);
